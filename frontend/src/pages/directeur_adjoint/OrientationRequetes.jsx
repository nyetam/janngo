...
export default function OrientationRequetes() {
  ...
  const aOrienter = filtree.filter(
    (r) => r.type === 'ATTESTATION' && r.statut === 'EN_COURS' && !r.attestation?.serviceTraitant
  );
  ...
  return (
    <Layout title="Attestations — Orientation">
          ...
          <div className="flex rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
            {ONGLETS.map((label, i) => (
              <button
                key={i}
                onClick={() => setOnglet(i)}
                className={`px-4 md:px-5 py-2.5 text-sm font-medium transition-colors flex items-center gap-2 ${
                  onglet === i ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
                {i === 0 ? '📤' : '📋'}
          ...
          <>
            {loading ? (
              ...
              <div className="card text-center py-16 text-gray-400">
                ...
                <button onClick={() => setOnglet(1)} className="text-blue-500 text-sm hover:underline mt-2 inline-block">
                  Voir l'historique →
                </button>
              </div>
            ) : (
                  ...
                  {selected?.id === r.id && (
                    <div className="border-t border-blue-100 pt-4 -mx-6 px-6 pb-4 bg-blue-50/30 rounded-b-xl space-y-3">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        <button onClick={handleOrienter} disabled={actionLoading} className="btn-primary">
                          {actionLoading ? (
                    ...
                    <div key={r.id} className="card">
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="min-w-0">
                          ...
                          {r.attestation && (
                            <p className="text-sm mt-1 text-blue-700">{r.attestation.typeAttestation}</p>
                          )}
                          {r.attestation?.serviceTraitant && (
                            <div className="mt-2 flex items-center gap-2 flex-wrap">
                              ...
                              {orientation && (
                                <span className="text-xs text-gray-400">{formatDateTime(orientation.dateChangement)}</span>
                              )}
                            </div>
                          )}
                        </div>
