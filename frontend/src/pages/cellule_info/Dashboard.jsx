...
export default function CelluleInfoDashboard() {
  ...
  // Cellule info : CORRECTION_NOM + CONTESTATION_NOTE validées → modifications système à faire
  const aModifier = liste.filter((r) =>
    ['CORRECTION_NOM', 'CONTESTATION_NOTE'].includes(r.type) && r.statut === 'VALIDEE'
  );
  // Clôturées (modifications effectuées dans le système)
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        <Navbar title="Cellule Informatique" />
                      ...
                      <div>
                        ...
                        {r.type === 'CONTESTATION_NOTE' && r.contestationNote && (
                          <p className="text-xs text-purple-600">
                            {r.contestationNote.matiere} · Note corrigée : {r.contestationNote.noteCorrigee ?? '?'}/20
                          </p>
                        )}
                      </div>
                    </div>
