...
// ─── Génération du modèle Excel vide ──────────────────────────────────────
...
function exporterRapportCSV(rapport) {
  ...
  const lignesOK = rapport.details.map((d) => [
    d.matricule, d.action, d.nom || '', d.email || '', 'OK',
  ]);
  const lignesErr = rapport.details_erreurs.map((d) => [
    d.matricule, 'ERREUR', '', '', d.raison,
  ]);
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        <Navbar title="Import des étudiants" />
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {apercu.apercu.map((e, i) => (
                        <tr key={i} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-2.5 font-mono text-xs text-amber-700 bg-amber-50"
                              title="Mot de passe masqué — seuls les 4 premiers caractères sont visibles">
                            {e.passwordApercu || '****'}
                          </td>
          ...
          {etape === ETAPES.RAPPORT && rapport && (
            <div className="space-y-5">
              ...
              <div className={`card text-center py-8 ${
                rapport.erreurs === 0 ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200'
              }`}>
